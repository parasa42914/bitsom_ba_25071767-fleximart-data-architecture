...
def clean_date(date_str):
    ...
    return pd.to_datetime(date_str, dayfirst=False, errors='coerce').date()
...
def run_etl():
    ...
    cust_df['email'] = cust_df.apply(lambda r: f"{r['first_name'].lower()}@fleximart.com" if pd.isna(r['email']) else r['email'], axis=1)
    ...
    # Prep for DB (Mapping C001 -> 1)
    sales_df['customer_id'] = sales_df['customer_id'].str.extract('(\d+)').astype(int)
    sales_df['product_id'] = sales_df['product_id'].str.extract('(\d+)').astype(int)

    # --- 3. LOADING ---
    try:
        cust_df.drop(columns=['customer_id']).to_sql('customers', engine, if_exists='append', index=False)
        prod_df.drop(columns=['product_id']).to_sql('products', engine, if_exists='append', index=False)
...
====================================================="""
    ...
    print("ETL Job Finished. Report generated in data_quality_report.txt")
